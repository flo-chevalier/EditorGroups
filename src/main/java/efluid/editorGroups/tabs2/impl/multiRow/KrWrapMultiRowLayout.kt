...
class KrWrapMultiRowLayout(tabs: KrTabsImpl, showPinnedTabsSeparately: Boolean) : KrMultiRowLayout(tabs, showPinnedTabsSeparately) {
    ...
    private fun doSplitToRows(
        ...
        infosToSplit: List<_root_ide_package_.efluid.editorGroups.tabs2.KrTabInfo>,
        getRowMaxLen: (index: Int) -> Int
    ) {
        var curRowInfos = mutableListOf<_root_ide_package_.efluid.editorGroups.tabs2.KrTabInfo>()
        var curLen = 0
    ...
    private fun createRow(infos: List<_root_ide_package_.efluid.editorGroups.tabs2.KrTabInfo>, isFirst: Boolean): KrTabsRow {
