...
public class KrDragHelper extends MouseDragHelper<KrTabsImpl> {
  ...
  protected void processMousePressed(@NotNull MouseEvent event) {
    ...
    KrTabLabel label = findLabel(new RelativePoint(event).getPoint(myTabs));
  ...
  protected void processDrag(@NotNull MouseEvent event, @NotNull Point targetScreenPoint, @NotNull Point startPointScreen) {
    ...
    if (isDragJustStarted()) {
      KrTabLabel pressedTabLabel = SoftReference.dereference(myPressedTabLabel);
    ...
    final KrTabLabel top = findLabel(new Point(dragRec.x + dragRec.width / 2, dragRec.y + deadZoneY));
    final KrTabLabel bottom = findLabel(new Point(dragRec.x + dragRec.width / 2, dragRec.y + dragRec.height - deadZoneY));
    final KrTabLabel left = findLabel(new Point(dragRec.x + deadZoneX, dragRec.y + dragRec.height / 2));
    final KrTabLabel right = findLabel(new Point(dragRec.x + dragRec.width - deadZoneX, dragRec.y + dragRec.height / 2));


    KrTabLabel targetLabel;
  ...
  private KrTabLabel findMostOverlapping(Axis measurer, KrTabLabel... labels) {
    double freeSpace;
    ...
    int max = -1;
    KrTabLabel maxLabel = null;
    for (KrTabLabel each : labels) {
  ...
  @Nullable
  private KrTabLabel findLabel(Component c) {
    Component eachParent = c;
    while (eachParent != null && eachParent != myTabs) {
      if (eachParent instanceof KrTabLabel) return (KrTabLabel) eachParent;
  ...
  protected boolean canFinishDragging(@NotNull JComponent component, @NotNull RelativePoint point) {
    ...
    if (myTabs.getVisibleInfos().isEmpty() && realDropTarget != null) {
      KrTabsImpl tabs = UIUtil.getParentOfType(KrTabsImpl.class, realDropTarget);
