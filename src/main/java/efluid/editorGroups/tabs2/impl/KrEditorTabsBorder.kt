...
import efluid.editorGroups.tabs2.KrTabsBorder
import java.awt.*
...
class KrEditorTabsBorder(tabs: KrTabsImpl) : KrTabsBorder(tabs) {
  ...
  init {
    tabs.addListener(object : _root_ide_package_.efluid.editorGroups.tabs2.KrTabsListener {
      ...
      private fun bounds(tabInfo: _root_ide_package_.efluid.editorGroups.tabs2.KrTabInfo?): Rectangle? {
  ...
  override fun paintBorder(c: Component, g: Graphics, x: Int, y: Int, width: Int, height: Int) {
    ...
    when (tabs.position) {
      _root_ide_package_.efluid.editorGroups.tabs2.KrTabsPosition.top -> {
        val startY = firstLabel.y - if (tabs.position == _root_ide_package_.efluid.editorGroups.tabs2.KrTabsPosition.bottom) 0 else thickness
        ...
        if (!ExperimentalUI.isNewUI() || (tabs as? _root_ide_package_.efluid.editorGroups.tabs2.impl.KrEditorTabs)?.shouldPaintBottomBorder() == true) {
      ...
      _root_ide_package_.efluid.editorGroups.tabs2.KrTabsPosition.right -> {
        val lx = firstLabel.x
