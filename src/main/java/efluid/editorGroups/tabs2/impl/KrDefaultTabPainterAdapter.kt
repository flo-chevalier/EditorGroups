...
class KrDefaultTabPainterAdapter(private val painter: KrTabPainter) : KrTabPainterAdapter {
    ...
    override fun paintBackground(label: _root_ide_package_.efluid.editorGroups.tabs2.impl.KrTabLabel, g: Graphics, tabs: KrTabsImpl) {
        val info = label.info
        ...
        if (isSelected && tabs.getVisibleInfos().size > 1) {
            painter
                .paintSelectedTab(
                    tabs.position, g2d, rect, tabs.borderThickness, info.tabColor, tabs.isActiveTabs(info),
