...
import efluid.editorGroups.actions.PopupMenu;
import efluid.editorGroups.actions.RemoveFromCurrentFavoritesAction;
import efluid.editorGroups.language.EditorGroupsLanguage;
import efluid.editorGroups.model.AutoGroup;
import efluid.editorGroups.model.BookmarkGroup;
import efluid.editorGroups.model.EditorGroup;
import efluid.editorGroups.model.EditorGroupIndexValue;
import efluid.editorGroups.model.EditorGroups;
import efluid.editorGroups.model.FavoritesGroup;
import efluid.editorGroups.model.GroupsHolder;
import efluid.editorGroups.model.HidePanelGroup;
import efluid.editorGroups.model.Link;
import efluid.editorGroups.model.PathLink;
import efluid.editorGroups.model.VirtualFileLink;
import efluid.editorGroups.support.FileResolver;
import efluid.editorGroups.support.Utils;
import efluid.editorGroups.tabs2.KrTabInfo;
import efluid.editorGroups.tabs2.KrTabs;
import efluid.editorGroups.tabs2.my.MyJBEditorTabs;


public class EditorGroupPanel extends JBPanel implements Weighted, Disposable {

  public static final DataKey<FavoritesGroup> FAVORITE_GROUP = DataKey.create("efluid.FavoritesGroup");
...
//		groupsPanel.setLayout(new HorizontalLayout(0));
      ...
      public void mouseReleased(MouseEvent e) {
        if (UIUtil.isCloseClick(e, MouseEvent.MOUSE_RELEASED)) {
          ...
          if (info != null) {
            ...
            try {
              ActionManager.getInstance().getAction(RemoveFromCurrentFavoritesAction.ID)
                           .actionPerformed(AnActionEvent.createFromInputEvent(e, ActionPlaces.UNKNOWN, new Presentation(), DataManager.getInstance().getDataContext(tabs)));
            } finally {
    ...
    myTaskExecutor = AppExecutorUtil.createBoundedApplicationPoolExecutor("efluid.editorGroups.EditorGroupPanel-" + file.getName(), 1);
  ...
  private void addButtons() {
    ...
    actionGroup.add(ActionManager.getInstance().getAction("efluid.editorGroups.Refresh"));
    actionGroup.add(ActionManager.getInstance().getAction("efluid.editorGroups.SwitchGroup"));

    toolbar = ActionManager.getInstance().createActionToolbar("efluid.editorGroups.EditorGroupPanel", actionGroup, true);
...
//			}
  ...
  public boolean previous(boolean newTab, boolean newWindow, Splitters split) {
    if (currentIndex == NOT_INITIALIZED) { //group was not refreshed
      if (LOG.isDebugEnabled()) {
        LOG.debug("openFile fail - currentIndex == -1");
      }
      return false;
    }
    if (displayedGroup.isInvalid()) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("openFile fail - displayedGroup.isInvalid");
      }
      return false;
    }
    if (!isVisible()) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("openFile fail - !isVisible()");
      }
      return false;
    }
  ...
  private boolean openFile(@Nullable Link link, boolean newTab, boolean newWindow, Splitters split) {
    ...
    if (link == null) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("openFile fail - link is null");
      }
      return false;
    }
    ...
    if (link.getVirtualFile() == null) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("openFile fail - file is null for " + link);
      }
      return false;
    }
  ...
  public void refreshOnSelectionChanged(boolean refresh, EditorGroup switchingGroup, int scrollOffset) {
    if (LOG.isDebugEnabled()) {
      LOG.debug("refreshOnSelectionChanged");
    }
  ...
  private RefreshRequest getGroupInReadActionWithRetries(Ref<EditorGroup> editorGroupRef) {
    ...
    while (!success) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("getGroupInReadActionWithRetries - " + request);
      }
      ...
      try {
        EditorGroup editorGroup = ReadAction.nonBlocking(() -> {
          try {
            ...
            if (LOG.isDebugEnabled()) {
              LOG.debug("getGroupInReadActionWithRetries - " + e, e);
            }
            throw e;
