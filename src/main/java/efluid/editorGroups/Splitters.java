...
public enum Splitters {
  ...
  public static Splitters from(InputEvent e) {
    return from(BitUtil.isSet(e.getModifiersEx(), InputEvent.ALT_DOWN_MASK));
  }

  public static Splitters from(AnActionEvent e) {
    boolean alt = BitUtil.isSet(e.getModifiers(), InputEvent.ALT_DOWN_MASK);
    boolean shift = BitUtil.isSet(e.getModifiers(), InputEvent.SHIFT_DOWN_MASK);
    if (alt && shift) {
      return HORIZONTAL;
    }
    return from(alt);
