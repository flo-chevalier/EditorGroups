...
import efluid.editorGroups.model.FolderGroup;
import efluid.editorGroups.model.RegexGroup;
import efluid.editorGroups.model.SameNameGroup;
import efluid.editorGroups.support.Notifications;
...
public class EditorGroupManager {
  ...
  public static final Comparator<EditorGroup> COMPARATOR = Comparator.comparing(o -> o.getTitle().toLowerCase());

  private final Project project;
  public IndexCache cache;

  private final ApplicationConfigurationComponent config = ApplicationConfigurationComponent.getInstance();
  private final PanelRefresher panelRefresher;
  private final IdeFocusManager ideFocusManager;
  private boolean warningShown;
  private final ExternalGroupProvider externalGroupProvider;
  private final AutoGroupProvider autogroupProvider;
  private final RegexGroupProvider regexGroupProvider;
  ...
  public static EditorGroupManager getInstance(@NotNull Project project) {
    return project.getService(EditorGroupManager.class);
  }
...
//		}
  ...
  private boolean isIndexingAutoGroup(Project project, EditorGroup result) {
    if (result instanceof AutoGroup s && !DumbService.isDumb(project)) {
      return s.hasIndexing();
    }
    return false;
  }
  ...
  public void enableSwitching() {
    SwingUtilities.invokeLater(() -> ideFocusManager.doWhenFocusSettlesDown(() -> {
  ...
  private Result open2(EditorWindow currentWindowParam, @Nullable VirtualFile currentFile, VirtualFile fileToOpen, Integer line, EditorGroup group, boolean newWindow, boolean newTab, Splitters split, SwitchRequest switchRequest) {
    ...
    CommandProcessor.getInstance().executeCommand(project, () -> {
      ...
      if (!split.isSplit() && !newWindow && fileToOpen.equals(selectedFile)) {
        FileEditor editors = Objects.requireNonNull(currentWindow).getManager().getSelectedEditor(fileToOpen);
