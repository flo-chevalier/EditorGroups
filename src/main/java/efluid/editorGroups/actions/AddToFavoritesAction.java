  ...
  Collection<AbstractTreeNode<?>> createNodes(Project project,
    ...
    if (object instanceof PsiElement) {
      ...
      if (inProjectView && ProjectView.getInstance(project).isShowModules(currentViewId)) {
        if (pane != null && pane.getSelectedPath() != null &&
          Objects.requireNonNull(TreeUtil.getLastUserObject(NodeDescriptor.class, pane.getSelectedPath())).getElement() instanceof AbstractTreeNode abstractTreeNode) {
